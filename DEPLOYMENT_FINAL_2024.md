...
### 1. 셔터음 재생 ✅
- ✅ iOS 시스템 셔터음 사용 (`AudioServicesPlaySystemSound(1108)`)
- ✅ `AudioToolbox` import 추가
- ✅ 촬영 시 자동 재생

### 2. 카메라 프리뷰 영역 복구 ✅
- ✅ `_buildCameraScreen()` 구조 수정
- ✅ `Positioned.fill`로 전체 영역 차지 보장
- ✅ 카메라 프리뷰가 정상적으로 렌더링됨

### 3. EXIF 방향 정규화 단일화 ✅
- ✅ HomePage에서 EXIF 정규화 제거
- ✅ FilterPage에서만 EXIF 정규화 수행
- ✅ 프리뷰/저장 모두 정규화된 이미지 사용
...
- [ ] **카메라 프리뷰 영역 테스트**
  - 카메라 프리뷰가 화면에 정상적으로 표시되는지
  - 비율 변경 (9:16, 3:4, 1:1) 시 프리뷰가 정상 작동하는지
...
2. **lib/pages/home_page.dart**
   - 카메라 프리뷰 영역 복구
   - EXIF 정규화 제거 (FilterPage에서만 수행)

3. **lib/pages/filter_page.dart**
   - EXIF 정규화 단일화
   - `_normalizeInitialImageAndLoad()` 메서드 추가
